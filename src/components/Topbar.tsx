...
export default function Topbar() {
  ...
  return (
    <header className="bg-card fixed inset-x-0 top-0 z-50 flex h-14 items-center justify-between px-3">
      <Link
        href={"/dashboard"}
        className="absolute left-0 flex items-center justify-center font-bold"
        style={{ width: "13.5rem" }}
      >
        <span className="relative">
          Virtual<span className="text-shy-moment">Toolkit</span>
          <span className="bg-shy-moment/50 absolute -right-2 -bottom-1 -left-2 h-px rounded-full" />
        </span>
      </Link>
